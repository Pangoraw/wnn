...
use wnn::{
    onnx,
    tensor::{CPUTensor, CPUTensorData, DataType},
    CompiledModel,
};

#[test]
fn test_resnet() -> anyhow::Result<()> {
    let filename = std::path::PathBuf::from("../wnn-wasm/resnet.onnx");
    ...
    let inputs = graph
        ...
        .map(|input| -> anyhow::Result<Vec<u8>> {
            ...
            if !matches!(dtype, DataType::F32) {
                bail!("model does not take f32 as input but {}", dtype);
            }
    ...
    match output.iter().next() {
        Some((
            _,
            CPUTensor {
                data: CPUTensorData::F32(data),
                ..
            },
        )) => {
            let mut max = f32::MIN;
            ...
            let class = CLASSES[argmax];
            let expected = "letter opener, paper knife, paperknife";

            if class != expected {
                bail!("invalid prediction \"{class}\", expected \"{expected}\"");
            }
